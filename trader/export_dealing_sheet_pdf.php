...
class DealingSheetPDF extends TCPDF {
    ...
    public function addDealingSheet($sheet, $fees, $exportedByName) {
        ...
        // ============================================
        // 4. FEES AND CHARGES - UPDATED FOR SHARES
        // ============================================
        $this->SetFont('helvetica', 'B', 11);
        $this->SetFillColor(240, 240, 240);
        ...
        $this->Cell(40, 7, 'Rate / Basis', 0, 0, 'R');
        ...
        if ($is_bond) {
            $this->Cell(40, 6, 'Tiered (0.063132% / 0.035%)', 0, 0, 'R');
        } else {
            $this->Cell(40, 6, 'Tiered (1.7% / 1.5% / 0.8%)', 0, 0, 'R');
        }
        ...
        $this->Cell(40, 6, '@ ' . $cmsa_rate . ' ' . $cmsa_basis, 0, 0, 'R');
        ...
        $this->Cell(100, 6, $cds_label, 0, 0, 'L');
        $this->Cell(40, 6, '@ ' . $cds_rate . ' ' . $cds_basis, 0, 0, 'R');
        $this->Cell(40, 6, number_format($fees['csd'], 2), 0, 1, 'R');
        
        // VRF (Equity/ETF only)
        if (!$is_bond) {
            $this->Cell(100, 6, 'VRF Fee', 0, 0, 'L');
            $this->Cell(40, 6, '@ 0.0025% of Consideration', 0, 0, 'R');
        ...
        if ($executed_qty > 0 && $executed_price > 0) {
            if ($is_bond) {
                $consideration = ($executed_price / 100) * $executed_qty;
            } else {
                $consideration = $executed_qty * $executed_price;
            }
        } else {
            if ($is_bond) {
                $consideration = ($price / 100) * $qty;
            } else {
...
// ============================================
// FEES CALCULATION FUNCTION - UPDATED FOR SHARES
// ============================================
