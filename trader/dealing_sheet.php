...
if (isset($_POST['approve_receipt']) && isset($_POST['trade_id'])) {
    ...
    if (!isAccountant($current_user)) {
        $_SESSION['alert'] = ['Only accountants can approve receipts.', 'danger'];
        header('Location: mtp_receipt_upload.php?' . http_build_query(array_filter([
            'filter' => $_GET['filter'] ?? 'pending',
            'asset_class' => $_GET['asset_class'] ?? 'all',
            'search' => $_GET['search'] ?? ''
    ...
    if ($result) {
        $_SESSION['alert'] = [
            $is_approved ? 'Receipt approved successfully!' : 'Receipt rejected. Comment saved.',
            $is_approved ? 'success' : 'warning'
        ];
    } else {
        $_SESSION['alert'] = ['Failed to update approval status.', 'danger'];
    }
...
<head>
    ...
    <style>
        ...
        * {
        ...
        .action-btn.approve:hover { background: #D1FAE5; color: var(--success); }
        .action-btn.reject:hover { background: #FEE2E2; color: var(--danger); }
    ...
    <div class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-2">
        <div>
            ...
            <p class="text-muted mb-0 small">
                Manage receipts for MTP payments (Additional Reference = Number only)
            </p>
        </div>
        <div>
                    ...
                    <tbody>
                        <?php foreach ($trades as $trade): 
                            ...
                            if (!empty($trade['comments'])) {
                                ...
                                for ($i = 0; $i < count($comment_parts); $i++) {
                                    $comments[] = [
                            ...
                            <tr>
                                ...
                                <td>
                                    <?php if ($hasReceipt): ?>
                                        <div class="receipt-thumbnails">
                                            ...
                                            <?php endif; ?>
                                            <?php if (isAccountant($current_user) || $isApproved === 0): ?>
                                        ...
                                        <div class="comment-bubble">
                                            <?php foreach ($comments as $c): ?>
                                                <div>
                                                    ...
                                                    <?php echo htmlspecialchars(substr($c['text'], 0, 50)); ?>
                                                    <?php if (strlen($c['text']) > 50): ?>...<?php endif; ?>
                                                </div>
                                            <?php endforeach; ?>
                                        </div>
                                    <?php else: ?>
                                ...
                                <td class="text-end">
                                    <div class="d-flex gap-1 justify-content-end">
                                        ...
                                        <?php if (isAccountant($current_user) && $hasReceipt): ?>
                                            <?php if ($isApproved === 0): ?>
                                                <button class="action-btn approve" onclick="openApprovalModal(<?php echo $trade['id']; ?>, 1)" title="Approve">
                                                    <i class="bi bi-check-lg"></i>
                                                </button>
                                                <button class="action-btn reject" onclick="openApprovalModal(<?php echo $trade['id']; ?>, 0)" title="Reject">
                                                    <i class="bi bi-x-lg"></i>
                                                </button>
                                            <?php elseif ($isApproved === 2): ?>
                                                <button class="action-btn approve" onclick="openApprovalModal(<?php echo $trade['id']; ?>, 1)" title="Re-approve">
    ...
    <div class="modal-dialog modal-md">
        <div class="modal-content">
            <div class="modal-header" id="approvalModalHeader">
                <h5 class="modal-title"><i class="bi bi-check-circle me-2"></i>Approve Receipt</h5>
            ...
            <form method="POST" action="mtp_receipt_upload.php">
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        ...
                        <div class="form-text">Add notes about the receipt verification.</div>
                    </div>
                    
                    <div class="alert alert-warning" id="approvalWarning">
                        <i class="bi bi-exclamation-triangle me-2"></i>
                        <span id="approvalMessage">Are you sure you want to approve this receipt?</span>
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
...
function openApprovalModal(tradeId, isApproved) {
    ...
    if (isApproved === 1) {
        ...
        msg.textContent = 'Are you sure you want to APPROVE this receipt?';
        warning.className = 'alert alert-success';
        warning.innerHTML = '<i class="bi bi-check-circle me-2"></i> ' + msg.textContent;
    } else {
        header.className = 'modal-header bg-danger text-white';
        btn.className = 'btn btn-danger';
        btn.textContent = 'Reject';
        msg.textContent = 'Are you sure you want to REJECT this receipt?';
        warning.className = 'alert alert-danger';
        warning.innerHTML = '<i class="bi bi-x-circle me-2"></i> ' + msg.textContent;
    }
...
class DealingSheetPDF extends TCPDF {
    ...
    public function Header() {
        // Render the report header (this appears at the top)
        $this->renderReportHeader();
        
        // Get the Y position after header
        $y = $this->GetY();
