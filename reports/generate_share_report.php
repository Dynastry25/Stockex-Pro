...
generateTransactionSummaryReport($db, $filters);

function generateTransactionSummaryReport($db, $filters) {
    ...
    $where_conditions = ["t.status = 'active'", "(t.asset_class = 'equity' OR t.asset_class = 'Exchange Traded Fund')"];
    $params = [];
    ...
    // Get all trades for the detailed transaction list
    $trades_query = "
        SELECT 
            t.*,
            b.broker_code,
    ...
    $pdf->SetTitle('Equities & ETF Transactions Summary Report');
    $pdf->SetSubject('Share Transactions Report');
    ...
    $pdf->Cell(0, 5, 'EQUITIES & ETF TRANSACTIONS SUMMARY REPORT FOR TRADES ON ' . $trade_date_display, 0, 1, 'C');
    ...
    // Report explanation
    $pdf->writeHTML('
    <div style="background-color: #f0f4f8; border-left: 4px solid #28a745; padding: 8px 12px; margin: 5px 0 10px 0; font-size: 8px; color: #333;">
        <strong>Report Overview:</strong> This Equities Transaction Summary provides a comprehensive overview of all share/equity trading activity 
        ...
        <strong>Purpose:</strong> Use this for regulatory reporting, management review, and performance analysis of the equities desk.
    </div>
    ...
    if (empty($display_trades)) {
        ...
        $pdf->Cell($total_cols_width, 8, 'No transactions found for the selected filters.', 1, 0, 'C');
        $pdf->Ln();
    } else {
    ...
    $pdf->SetFont('helvetica', 'B', 11);
    $pdf->Cell(0, 5, 'Division : ' . strtoupper($company_division), 0, 1, 'L');
    $pdf->Cell(0, 5, 'BROKER SUMMARY (EQUITIES & EXCHANGE TRADED FUNDS)', 0, 1, 'C');
    $pdf->Ln(3);
    
    // Get broker summary for counterpart brokers (brokers trading with our clients)
    // This gets brokers that appear as counterparty in trades with our clients
    $broker_summary_query = "
        ...
        AND t.status = 'active' 
        AND (t.asset_class = 'equity' OR t.asset_class = 'Exchange Traded Fund')
    ...
    // Add a new page for custodian summary
    $pdf->AddPage();
    
    // Custodian summary header
    $pdf->SetFont('helvetica', 'B', 11);
    $pdf->Cell(0, 5, 'Division : ' . strtoupper($company_division), 0, 1, 'L');
    
    $settlement_date_display = !empty($filters['period_to']) ? 
        date('d/m/Y', strtotime($filters['period_to'])) : 
        date('d/m/Y');
    
    $pdf->Cell(0, 5, 'CUSTODIAN WISE EQUITIES TRANSACTIONS SUMMARY REPORT FOR SETTLEMENTS ON ' . $settlement_date_display, 0, 1, 'C');
    
    $pdf->SetFont('helvetica', '', 9);
    $pdf->Cell(0, 4, 'Date : ' . date('d/m/Y') . ' : ' . date('H:i:s'), 0, 1, 'C');
    $pdf->Ln(3);
    
    // Get custodian summary data
    $custodian_summary_query = "
        SELECT 
            ct.custodian_code,
            ct.custodian_name,
            -- Inflow: When broker is BUYING from custodian client (broker pays money)
            COALESCE(SUM(CASE 
                WHEN ct.trade_side = 'buy' AND ct.asset_class IN ('equity', 'Exchange Traded Funds')
                THEN ct.consideration 
                ELSE 0 
            END), 0) as inflow,
            -- Outflow: When broker is SELLING to custodian client (broker receives money)
            COALESCE(SUM(CASE 
                WHEN ct.trade_side = 'sell' AND ct.asset_class IN ('equity', 'Exchange Traded Funds')
                THEN ct.consideration 
                ELSE 0 
            END), 0) as outflow,
            -- Net: Inflow - Outflow
            ...
            COALESCE(SUM(CASE 
                ...
                ELSE 0 
            END), 0) as net
        FROM custodians_trades ct
        WHERE ct.asset_class IN ('equity', 'Exchange Traded Funds')
        " . ($filters['period_from'] ? "AND ct.trade_date >= :period_from" : "") . "
        " . ($filters['period_to'] ? "AND ct.trade_date <= :period_to" : "") . "
        GROUP BY ct.custodian_code, ct.custodian_name
        HAVING inflow != 0 OR outflow != 0
        ORDER BY ct.custodian_code
    ";
    
    $custodian_stmt = $db->prepare($custodian_summary_query);
    
    if ($filters['period_from']) {
        $custodian_stmt->bindValue(':period_from', $filters['period_from']);
    }
    if ($filters['period_to']) {
        $custodian_stmt->bindValue(':period_to', $filters['period_to']);
    }
    
    $custodian_stmt->execute();
    $custodian_summary = $custodian_stmt->fetchAll(PDO::FETCH_ASSOC);
    
    // Calculate custodian summary totals
    $custodian_total_inflow = 0;
    $custodian_total_outflow = 0;
    $custodian_total_net = 0;
    
    foreach ($custodian_summary as $custodian) {
        $custodian_total_inflow += $custodian['inflow'];
        $custodian_total_outflow += $custodian['outflow'];
        $custodian_total_net += $custodian['net'];
    }
    
    // Custodian summary table header
    $custodian_col_widths = [
        '#' => 5,
        'code' => 12,
        'name' => ($orientation === 'L') ? 70 : 55,
        'inflow' => 25,
        'outflow' => 25,
        'net' => 25,
        'balance' => 25
    ...
    $pdf->Cell($custodian_col_widths['#'], 6, '#', 1, 0, 'C');
    $pdf->Cell($custodian_col_widths['code'], 6, 'Code', 1, 0, 'C');
    $pdf->Cell($custodian_col_widths['name'], 6, 'Name', 1, 0, 'C');
    $pdf->Cell($custodian_col_widths['inflow'], 6, 'Inflow', 1, 0, 'C');
    $pdf->Cell($custodian_col_widths['outflow'], 6, 'Outflow', 1, 0, 'C');
    $pdf->Cell($custodian_col_widths['net'], 6, 'Net', 1, 0, 'C');
    $pdf->Cell($custodian_col_widths['balance'], 6, 'Balance', 1, 0, 'C');
    $pdf->Ln();
    
    // Custodian summary data
    $pdf->SetFont('helvetica', '', 7);
    $custodian_counter = 1;
    $custodian_running_balance = 0;
    
    if (empty($custodian_summary)) {
        $total_custodian_width = array_sum($custodian_col_widths);
        $pdf->Cell($total_custodian_width, 8, 'No custodian data available for the selected period.', 1, 0, 'C');
        $pdf->Ln();
    } else {
        foreach ($custodian_summary as $custodian) {
            $custodian_running_balance += $custodian['net'];
            
            $custodian_name_chars = floor($custodian_col_widths['name'] / 1.5);
            $custodian_name_display = strlen($custodian['custodian_name']) > $custodian_name_chars ? 
                substr($custodian['custodian_name'], 0, $custodian_name_chars-3) . '...' : $custodian['custodian_name'];
            
            $pdf->Cell($custodian_col_widths['#'], $row_height, $custodian_counter, 1, 0, 'C');
            $pdf->Cell($custodian_col_widths['code'], $row_height, $custodian['custodian_code'], 1, 0, 'C');
            $pdf->Cell($custodian_col_widths['name'], $row_height, $custodian_name_display, 1, 0, 'L');
            $pdf->Cell($custodian_col_widths['inflow'], $row_height, number_format($custodian['inflow'], 2), 1, 0, 'R');
            $pdf->Cell($custodian_col_widths['outflow'], $row_height, number_format($custodian['outflow'], 2), 1, 0, 'R');
            $pdf->Cell($custodian_col_widths['net'], $row_height, number_format($custodian['net'], 2), 1, 0, 'R');
            $pdf->Cell($custodian_col_widths['balance'], $row_height, number_format($custodian_running_balance, 2), 1, 0, 'R');
            $pdf->Ln();
            $custodian_counter++;
        ...
        // Totals row for custodian summary
        $pdf->SetFont('helvetica', 'B', 7);
        $custodian_total_label_width = $custodian_col_widths['#'] + $custodian_col_widths['code'] + $custodian_col_widths['name'];
        $pdf->Cell($custodian_total_label_width, $row_height, 'CUSTODIAN TOTAL', 1, 0, 'C');
        $pdf->Cell($custodian_col_widths['inflow'], $row_height, number_format($custodian_total_inflow, 2), 1, 0, 'R');
        $pdf->Cell($custodian_col_widths['outflow'], $row_height, number_format($custodian_total_outflow, 2), 1, 0, 'R');
        $pdf->Cell($custodian_col_widths['net'], $row_height, number_format($custodian_total_net, 2), 1, 0, 'R');
        $pdf->Cell($custodian_col_widths['balance'], $row_height, number_format($custodian_total_net, 2), 1, 0, 'R');
        $pdf->Ln();
    }
    
    // Add custodian summary explanation
    $pdf->Ln(5);
    $pdf->SetFont('helvetica', 'I', 7);
    $pdf->MultiCell(0, 3, 'Custodian Summary Explanation:', 0, 'L');
    $pdf->MultiCell(0, 3, '1. Inflow: Total amount paid to custodians (broker buys from custodian clients)', 0, 'L');
    $pdf->MultiCell(0, 3, '2. Outflow: Total amount received from custodians (broker sells to custodian clients)', 0, 'L');
    $pdf->MultiCell(0, 3, '3. Net: Inflow minus Outflow', 0, 'L');
    $pdf->MultiCell(0, 3, '4. Positive Net: More money paid to custodians than received', 0, 'L');
    $pdf->MultiCell(0, 3, '5. Negative Net: More money received from custodians than paid', 0, 'L');
    ...
    // Output the PDF
    $filename = 'Equities_ETF_Custodian_Transactions_Summary_Report_' . date('d_m_Y_H_i_s') . '.pdf';
